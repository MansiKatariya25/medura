...
export default function AmbulanceAuth() {
    ...
    try {
      if (mode === "signup") {
        const res = await fetch("/api/auth/register/ambulance", {
          ...
          body: JSON.stringify({ ambulanceNumber, phoneNumber, riderName, email, password, location }),
        });
  ...
  return (
    <div className="min-h-screen bg-[#05060B] px-4 py-10 text-white">
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-8">
        <Card title={mode === "signup" ? "Ambulance Signup" : "Ambulance Signin"}>
          ...
          <form className="mt-5 space-y-4" onSubmit={handleSubmit}>
            {mode === "signup" ? (
              <>
                <Input label="Ambulance number" value={ambulanceNumber} onChange={(e) => setAmbulanceNumber(e.target.value)} required />
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <Input label="Rider name" value={riderName} onChange={(e) => setRiderName(e.target.value)} required />
                  <Input label="Phone number" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} required />
                </div>
                <Input label="Current location (auto)" value={location ?? ""} readOnly />
              </>
            ) : null}
